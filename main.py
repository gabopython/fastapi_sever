...
async def generate_auth_url(state: str, api_key: str):
    ...
    # FIX 1: Bypass Tweepy's wrapper to pass 'state'
    # Tweepy's .get_authorization_url() doesn't accept arguments.
    # We access the underlying requests_oauthlib session (.oauth) directly.
    # We must also pass the code_challenge manually here.
    authorization_url, _ = oauth2_handler.oauth.authorization_url(
        "https://twitter.com/i/oauth2/authorize",
        state=state,
        code_challenge=oauth2_handler.oauth.code_challenge,
        code_challenge_method=oauth2_handler.oauth.code_challenge_method
    )
    
    # FIX 2: Store the PKCE Code Verifier
    # The handler generates a random verifier. We MUST save this to use
    # it in the callback, otherwise the 'code' exchange will fail.
    # We access the private client attribute to get the verifier.
    code_verifier = oauth2_handler.oauth._client.code_verifier
